...
class BlendPlanner:
    ...
    def scale_blend(self, name, desired_plots):
        ...
        base_plots = blend["plots"]
        factor = desired_plots/base_plots

        return {
            mineral: round(amount * factor, 2)
    ...
    def check_stock(self, name, stock):
        ...
        for mineral, required in blend["minerals"].items():
            available = stock.get(mineral, 0)
            if available < required:
                missing[mineral] = round(required - available, 2)
